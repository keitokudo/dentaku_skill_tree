...
from dentaku_bart_base import DentakuBartBase

class DentakuBart(DentakuBartBase):
    @staticmethod
    def add_model_specific_args(parent_parser):
        parser = DentakuBartBase.add_model_specific_args(parent_parser)
    ...
    def __init__(self, config):
        ...
        if self.hparams.from_scratch:
            if not self.hparams.model_name_or_path.startswith("facebook/bart-large"):
                logger.warning(
                    "Although local pretrained model was specified, \"from_scratch\" is also selleted"
                )
                
            self.model_config = BartConfig.from_pretrained(self.hparams.model_name_or_path)
