...

{
  global_setting: {
    ...
    seed: 42,
    tag: "%s_from_scratch_seed_%s" % [std.extVar("TAG"), std.toString(self.seed)],
    log_model_output_dir: "%s/experiment_results/%s" % [ROOT_DIR, self.tag],
  },

  Logger: {
    project_name: "dentaku_skill_tree_public_from_scratch_seed_%s" % std.toString($.global_setting.seed),
